...
| `suppress-limit` | `50` | `[20, 200]` | Hard safety limit: consecutive same-direction events *absorbed* in COASTING (event speed at or below the current inertia, consumed silently) before forcing a transition back to TRACKING.  Bumps (event speed *above* inertia — the user driving the coast) don't count, so a strong flick's natural-decay tail can't cancel its own inertia.  `50` ≈ 400 ms at 125 Hz.  Too low causes drop-outs on valid long coasts; too high lets stuck states persist. |
...
| `exact-magnitude` | unset | `unset` / `set` | Opt-in boolean.  When unset (default), every vector-magnitude computation uses the integer approximation max(\|x\|,\|y\|) + min/2 (up to ~12% over the true magnitude on diagonals, plenty accurate for a threshold).  When set, routes through `√(x²+y²)` (integer Newton's sqrt, no libm) so `start` / `stop` / `decel-ratio` / `suppress-limit` and the cross-axis merge become strictly angle-invariant.  Expect to trim `start` by a few percent after enabling, since diagonal flicks now compare against their true (slightly lower) magnitude. |
...
A `suppress_count` counts consecutive same-direction events *absorbed* during COASTING (event speed ≤ inertia, consumed silently); after `suppress-limit` (default `50` ≈ 400 ms at 125 Hz) the processor drops back to TRACKING on the assumption that the user has been rolling through a stale coast.
Bumps (event speed *above* inertia) are deliberately excluded from the count — those represent the user driving the coast, the opposite of the stuck state this safety detects, so a strong flick's long natural-decay tail can't trip the limit and cancel its own inertia.
